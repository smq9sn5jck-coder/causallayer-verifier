...
export async function verifyCertificate(input) {
    ...
    try {
        ...
        const missing = required.filter((f) => !(f in cert));
        // Validate field types of untrusted input before any crypto/structural checks consume them.
        const typeErrors = [];
        if (cert.issuer !== undefined && (typeof cert.issuer !== "object" || cert.issuer === null || Array.isArray(cert.issuer)))
            typeErrors.push("issuer must be an object");
        if (cert.signature !== undefined && typeof cert.signature !== "string")
            typeErrors.push("signature must be a string");
        if (cert.merkle_root !== undefined && typeof cert.merkle_root !== "string")
            typeErrors.push("merkle_root must be a string");
        if (cert.causal_chain !== undefined && !Array.isArray(cert.causal_chain))
            typeErrors.push("causal_chain must be an array");
        if (cert.outputs !== undefined && (typeof cert.outputs !== "object" || cert.outputs === null || Array.isArray(cert.outputs)))
            typeErrors.push("outputs must be an object");
        if (missing.length > 0) {
    ...
    try {
        ...
        delete signableFields.signature;
        delete signableFields.anchor;
    ...
    try {
        ...
        const leaves = chain.map((node) => canonicalize(node));
